...
@ai_fn(instructions="You are an accurate and experienced copywriter.")
...
@ai_fn(
    ...
    tools=[
        ...
        marvin.tools.shell.Shell(require_confirmation=True, working_directory=FILE_DIR),
    ],
)
def summarize_from_file(file_name: str) -> str:
    f"""You are responsible for writing a summary of the text given in the
    text file {file_name}, located at {FILE_DIR}. You are not allowed to write
    or modify any files. You are only allowed to read files from {FILE_DIR}.
...
async def upload_file(file: UploadFile | None = None):
    ...
    print(summary_str)
    return TextSummary(file_name=file.filename, summary=summary_str)
...
class QAState(BaseModel):
    history: list[QAResponse] = Field(
        ...
        description=(
            """A place to record the history of questions and 
            answers concerning the content of one given file.
            """
        ),
    )
...
def run(question: str) -> str:
    qa = AIApplication(
        ...
        # history=
        description=f"""
        You are a chatbot answering to to all questions concerning the content of a 
        given text file.
        The text file has the name {"test2.txt"} and is located at {FILE_DIR}.
        Please make the embeddings for the content of the text file only once and 
        save them into a chroma vector database, which should be saved for later use in
        {FILE_DIR} .
        ...
        Make shure you always reuse the embeddings from the existing vector database 
        as knowledgebase. When you write the answers, you will need to ensure that the
        user's expectations are met. Remember, you are an accurate and experianced 
        author and you write unique and short answers aligned with the content of the 
        given text file (which embeddings should be saved in the chroma vector 
        database).
        ...
        The last sentence should state if new embeddings were used or the embeddings 
        of the chroma data base and the path and file name of the chroma database.
        """,
        tools=[
            marvin.tools.filesystem.ReadFile(root_dir=FILE_DIR),
            ...
            marvin_QueryChroma(
                name="chroma_db_text",
                description="""chroma data base to store the embeddings of the content
                of the given text
                """,
            ),
